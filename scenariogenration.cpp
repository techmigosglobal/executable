/*
% This program is the Scenario Generator for DRDL checks.
% It generates Shooter & Target data required for DLZ and Seeker Checks
% Developed by B. Jaganmohan, Sc D, Project Astra
% Version 1.0
% Date May 31, 2017
% Date July 18, 2017 - The inbuilt function ecef2lla is replaced by user
% defined function
% The following assumptions have been made for generating the scenario:
% 1. Shooter is always heading in North with the Input velocity Vs
% 2. No climbing of shooter
% 3. No Shooter Aspect Angle
% 4. Radar has a coverage of +/- 70 Deg in both Azimuth & Elevation Planes
%    and the target data set becomes Zero beyond this limit
%--------------------------------------------------------------------------
% npi2pi Convert normalized angles to lie between -pi and pi
% zero22pi Convert normalized angles to lie between 0 and 2pi
*/
//clc;
//close all;
//clear all;
...
Hs = input('Enter Shooter height in km\n Range of values : 0 to 20 km : ');                 %in km
Vs = input('Enter Shooter velocity in m/s\n Range of values : 50 to 650 m/s : ');           %in m/s
Ht = input('Enter Target height in km\n Range of values : 0 to 20 km : ');                  %in km
Vt = input('Enter Target velocity in m/s\n Range of values : 50 to 650 m/s : ');            %in m/s
TAA = input('Enter Target Aspect Angle in Deg\n Range of values : -180 to 180 deg : ');     %in Deg
R = input('Enter simulation start range in km\n Range of values : 2 to 200 km : ');         %in km
SAA = 0;            %in Deg
SCA = 0;            %in Deg
%--------------------------------------------------------------------------
%Pune Lat = 18.5856136680;
%Pune Long = 73.9182666689;
%--------------------------------------------------------------------------
%Outputs:
%--------------------------------------------------------------------------
Ri   = 0.0;
...
if(Hs<0.0 || Hs>20.0)
    display('Shooter Height is out of specified range of 0 to 20 km');
    return;
end;

if(Ht<0.0 || Ht>20.0)
    display('Target Height is out of specified range of 0 to 20 km');
    return;
end;

if(Vs<50.0 || Vs>650.0)
    display('Shooter Velocity is out of specified range of 0 to 650 m/s');
    return;
end;

if(Vt<50.0 || Vt>650.0)
    display('Target Velocity is out of specified range of 0 to 650 m/s');
    return;
end;

if(TAA<-180.0 || TAA>180.0)
    display('Target Aspect Angle is out of specified range of -180 to 180 Deg');
    return;
end;

if(R<2.0 || R>200.0)
    display('Simulation start range is out of specified range of 2 to 200 km');
    return;
end;
...
////////////////
function val = proc_15sgnf(X,flength,SF)
