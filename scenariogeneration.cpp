...
#define cosd(x) cos(x)*rad2deg
#define sind(x) sin(x)*rad2deg
...
void senariogeneration()
{
    ...
    Vnti = -Vt*cosd(TAA)*0.001;//cosd

    Veti = -Vt*sind(TAA)*0.001;//dd
    Vupti = 0.0;
    ...
    CENU2ECEF = DCMENU2ECEF(Latsi,Longsi); //dd
    Geodetic2ECEF(Latsi,Longsi,Hsi);
...
void Geodetic2ECEF()
{
    ...
    double a = 6378137.0;
    double dr = 1.0 - esq * sind(Lat)^2;
    int N = a/sqrt(dr);

    XS_ECEFi = (N + h) * cosd(Lat) * cosd(Lon);
    YS_ECEFi = (N + h) * cosd(Lat) * sind(Lon);
    ZS_ECEFi = (N * (1-esq) + h) * sind(Lat);

}

int proc_15sgnf(int X, int flength, int SF)
{
    Y = round(X/SF);
